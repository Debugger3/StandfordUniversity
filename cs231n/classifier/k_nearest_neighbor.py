...
class KNearestNeighbor(object):
  ...
  def compute_distances_no_loops(self, X):
    ...
    #########################################################################
    #pass
    #dists=np.dot(self.X_train.T,self.X_train)+np.dot(X.T,X)-2*np.dot(X,self.X_train.T)
    #dists=X2+y2-2xy
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in xrange(num_test):
      ...
      #########################################################################
      #sortedDistIndex=np.argsort(dists[i,:])
      closest_y = self.y_train[np.argsort(dists[i,:])[:k]]
      #closest_y=self.y_train[np.where(sortedDistIndex[:k])].tolist()
      #pass
      #########################################################################
      # TODO:                                                                 #
