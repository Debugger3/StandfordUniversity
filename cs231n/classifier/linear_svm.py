...
def svm_loss_naive(W, X, y, reg):
  ...
  for i in xrange(num_train):
    ...
    for j in xrange(num_classes):
		...
		#difference between correct lable score and predicted class score  
...
def svm_loss_vectorized(W, X, y, reg):
  ...
  margins=np.maximum(scores.T-correct_class_scores+1,0)
  #print margins.shape
  margins[y,np.array(range(num_train))]=0
  loss=np.sum(margins)
  loss/=num_train
  ...
  #print margins.shape
  #print W.shape
  margins[y, np.arange(num_train)] = -1 * np.sum(margins, 0)
  dW = np.dot(margins, X)
  dW /= num_train
  #print dW.shape
  dW += reg * W.T
